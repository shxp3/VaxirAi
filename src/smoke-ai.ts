...
try {
  ...
  await queuedProvider(createProvider(env.defaultAI, env.customAllowedBaseUrls), env.providerRequestIntervalMs).generate([{ role: 'user', content: 'Reply with OK.' }], env.defaultAI, env);
  safeLog('smoke_ok');
