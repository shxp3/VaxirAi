...
export function resolveAI(env: Env, secrets: Secrets, settings: GuildSettings, guildId: string) {
  const config: ProviderConfig = settings.ai ? { provider: settings.ai.provider, model: settings.ai.model, apiKey: secrets.decrypt(settings.ai.encryptedKey, guildId), baseUrl: settings.ai.baseUrl, apiFormat: settings.ai.apiFormat } : env.defaultAI;
  ...
  return { provider: createProvider(config, env.customAllowedBaseUrls), config, grounder };
}
