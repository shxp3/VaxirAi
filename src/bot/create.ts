...
export function createBot(env: Env, conversations: Conversations, admin: AdminCommands, plans?: ServerPlans) {
  ...
  client.on(Events.InteractionCreate, async interaction => {
    ...
    try {
      ...
      if (!['ask', 'status'].includes(interaction.commandName) && !isAdmin(interaction.memberPermissions)) {
        await interaction.reply({ content: 'ผู้ใช้ทั่วไปใช้ได้เฉพาะ /ask และ /status คำสั่งนี้ต้องเป็นผู้ดูแลเซิร์ฟเวอร์', flags: MessageFlags.Ephemeral }); return;
      }
      if (['setup', 'status'].includes(interaction.commandName)) { await admin.handle(interaction); return; }
