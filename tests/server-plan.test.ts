...
test('plans accept 30 operations and reject 31', () => {
  const operations = Array.from({ length: 30 }, (_, index) => ({ action: 'create', name: `channel-${index}`, type: 'text', parent: null }));
  assert.equal(validateOperations(operations, channels, []).length, 30);
  assert.throws(() => validateOperations([...operations, { action: 'create', name: 'extra', type: 'text', parent: null }], channels, []), /1–30/);
});
